#--------------------------------------------------------------------------------------
#
#     $Source: Performance/ElementCRUD/ModifiedPerformanceScripts/GeneratePerformanceCrudReport.py $
#
...
parser.add_argument("--sourceRoot", help="Path to the source folder of the tree for which you ran performance", required=True)
...
#Finding current stream name
commonScriptsDir = os.path.join(SourceRoot, 'DgnDbTestingScripts' , 'CommonTasks')
print commonScriptsDir
sys.path.append(commonScriptsDir)
import FindStream
TeamConfigPath = os.path.join(os.getenv('SrcRoot'), 'teamConfig', 'treeConfiguration.xml')
activeStream = FindStream.FindStreamDetails(TeamConfigPath)
print activeStream
...
if AnalysisReport is None:
    print 'Analysis report not found. Exiting.'
    exit(1)
...
#Locating Json file 
JsonFile = os.path.join(SourceRoot , 'DgnPlatform' , 'Tests' , 'DgnProject', 'PerformanceTestData', 'ElementCrud.json')
...
cmd = 'python ' + os.path.join(SourceRoot, 'DgnDbTestingScripts' , 'Performance', 'ElementCRUD', 'ModifiedPerformanceScripts' , 'AddDataToArchiveDb.py') + ' ' + AnalysisReportPath  + ' ' +archiveDbPath + ' ' + str(SmallDbSize) + ' ' + str(MediumDbSize) + ' ' + str(LargeDbSize)
RunCommand(cmd)
